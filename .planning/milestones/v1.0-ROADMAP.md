...
**Goal:** Implement organization-level monetization via Stripe Checkout and Webhooks.
**Requirements:** BILL-01, BILL-02

**Success Criteria:**
1. Stripe Checkout session correctly references `orgId` in metadata.
2. Webhooks securely update the `organizations.plan` field on event completion.
3. Users can view their current plan and status in the Billing settings.

---

## Phase 7: Project Management (Tenant Isolation)
**Goal:** Implement the primary business entity (Projects) with strict schema-per-tenant isolation.
**Requirements:** PROJ-01

**Success Criteria:**
1. New projects are inserted explicitly into the tenant-specific schema.
2. `getProjectsAction` uses the `getTenantDb` helper to enforce the `search_path`.
3. UI correctly handles empty states and creation workflows per organization.
